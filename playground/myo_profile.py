...
def read_box_dense_world_data(data_path):
    ...
    for subdict in subdict_list:
        ...
        simulation_steps_path = subdict + "/_step.npy"
...
def draw_box_pushing_iqm(is_success, simulation_steps, algorithm, case):
    ...
    tikzplotlib.save(f"{case}_iqm.tex")
...
if __name__ == "__main__":
    ...
    # draw the iqm curve
    reshaped_is_success = np.reshape(eva_return, (-1, eva_return.shape[-1]))
    ...
    total_sample = 40
    down_sample = simulation_steps.shape[-1] // total_sample
    draw_box_pushing_iqm(smooth_reshaped_is_success[:, ::down_sample],
                         reshaped_simulation_steps[:, ::down_sample], None, task)